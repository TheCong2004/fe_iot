...
export default function RegisterPage() {
  ...
  const [scanningForMark, setScanningForMark] = useState<boolean>(false);
  ...
  // (Không sửa đoạn loadFaceApi vì không dùng any cho state/props)
  ...
  async function handleAutoMarkDetected(d: { dataUrl: string }){
    ...
    try {
      ...
      try {
        ...
        console.error(err);
        setMsg('Lỗi khi gọi /api/scan-and-mark: ' + (err.message || String(e)));
      }
    } catch (e) {
      const err = e as Error;
      console.error(err);
      setMsg('Lỗi khi so khớp: ' + (err.message || String(e)));
    }
  }
  ...
  async function handleSave() {
    ...
    try {
      ...
      const err = e as Error;
      console.error(err);
      setMsg('Lỗi khi lưu: ' + (err.message || String(e)));
    }
  }
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div>
            <div className="border rounded p-4">
              ...
              {capturedDataUrl ? (
                <img src={capturedDataUrl} alt="Ảnh preview" className="w-full rounded mb-3" />
              ) : (
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  {samples.map((s, i) => (
                    <div key={i} className="relative">
                      <img src={s} alt={`Sample ${i+1}`} className="w-20 h-20 object-cover rounded border" />
